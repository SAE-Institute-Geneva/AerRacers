...
{
public:
    explicit VertexInput(uint32_t binding = 0,
...
public:
    ...
    [[nodiscard]] virtual std::vector<Vertex> GetVertices(size_t offset) const;
    virtual void SetVertices(const std::vector<Vertex>& vertices);

    [[nodiscard]] virtual std::vector<uint32_t> GetIndices(size_t offset) const;
