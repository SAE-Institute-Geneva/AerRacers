...
void RigidActor::SetBoxColliderData(const physics::BoxColliderData& boxColliderData) const
{
	if (!shape_)
	{
		logDebug("No shape_");
	...
	physx::PxPhysics* physics, const PhysicsMaterial& material)
{
	return physics->createMaterial(
		material.staticFriction, material.dynamicFriction, material.bouciness);
}
...
RigidActorData RigidActorViewer::GetRigidActorFromJson(const json& rigidActorJson)
{
	...
	if (CheckJsonParameter(rigidActorJson, "sphereCollider", json::value_t::object))
	{
		if (CheckJsonParameter(rigidActorJson["sphereCollider"], "exist", json::value_t::boolean))
		{
			if (rigidActorJson["sphereCollider"]["exist"])
			{
				...
				if (CheckJsonNumber(rigidActorJson["sphereCollider"], "radius"))
	...
	if (CheckJsonParameter(rigidActorJson, "physicsMaterial", json::value_t::object))
	{
		if (CheckJsonNumber(rigidActorJson["physicsMaterial"], "bouciness"))
			rigidActorData.material.bouciness = rigidActorJson["physicsMaterial"]["bouciness"];
...
RigidActorData RigidActorViewer::DrawImGuiRigidActor(const RigidActorData& rigidActorData)
{
	...
	switch (newRigidActorData.colliderType)
	{
		...
		case neko::physics::ColliderType::SPHERE:
		{
			...
			{
				ImGui::DragFloat3("offset", sphereColliderData.offset.coord, 0.0f);
...
RigidStaticData RigidStaticManager::GetRigidStaticData(Entity entity) const
{
...
{
	if (!physicsEngine_.IsPhysicRunning()) GetComponent(entity).SetRigidStaticData(rigidStaticData);
}

void RigidStaticManager::AddComponent(Entity entity)
{
...
DynamicData RigidDynamicManager::GetDynamicData(Entity entity) const
{
...
void RigidDynamicViewer::SetComponentFromJson(Entity entity, const json& componentJson)
{
	...
	if (CheckJsonParameter(componentJson, "isKinematic", json::value_t::boolean))
		rigidDynamicData.isKinematic = componentJson["isKinematic"];
	...
	if (entityManager_.HasComponent(entity, EntityMask(ComponentType::RIGID_DYNAMIC)))
	{
				...
				lastSelectedEntity_, EntityMask(ComponentType::RIGID_DYNAMIC)))
		{
			if (ImGui::TreeNode("Rigid Dynamic"))
			{
				...
				ImGui::DragFloat3("linearVelocity", linearVelocity.coord, 0.0f);
				ImGui::DragFloat("linearDamping", &rigidDynamicData.linearDamping);
				if (rigidDynamicData.linearDamping < 0.0f) rigidDynamicData.linearDamping = 0.0f;

				neko::Vec3f angularVelocity = dynamicData_.angularVelocity;
				ImGui::DragFloat3("angularVelocity", angularVelocity.coord, 0.0f);
				ImGui::DragFloat("angularDamping", &rigidDynamicData.angularDamping);
				if (rigidDynamicData.angularDamping < 0.0f) rigidDynamicData.angularDamping = 0.0f;

				ImGui::DragFloat("mass", &rigidDynamicData.mass, 0.5f);
				if (rigidDynamicData.mass < 0.0f) rigidDynamicData.mass = 0.0f;
